...
使用此作为默认顶层模块集。跨项目保持模块名称和顺序稳定；在页面内容和子页面中适配项目特定细节。在覆盖矩阵中将真正缺失的模块标记为 `N/A`，而不是创建填充内容。
...
5. [测试、构建与发布](./08-testing-build-release/README.md)
6. [术语表](./glossary.md)
...
- `拆分` - 创建领域子页面并在 `coverage-matrix.md` 和 `source-map.md` 中添加匹配行。
- `保持` - 当前页面给予足够聚焦的覆盖。
- `合并` - 概念不足以独立成页。
...
- 在索引和覆盖矩阵中保持规范 Wiki 结构可见，以便不同项目的 Wiki 保持可比。
...
- 在编写散文之前填写覆盖矩阵，并在领域覆盖审计期间更新它；它应记录领域概念、当前覆盖、拆分建议、源码证据、测试证据、外部参考证据和优先级。
