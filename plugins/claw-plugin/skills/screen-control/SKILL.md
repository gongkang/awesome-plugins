...
name: screen-control
description: |
  智能屏幕控制 - 让 AI 像人类一样"看"屏幕并操作。

  【何时触发此技能 - 必须触发】
  当用户的请求涉及以下任一情况时，**必须立即触发此技能**，即使用户没有明确说"屏幕控制"或"自动化"：

  **核心触发场景**：
  - **GUI 操作**："帮我点击..."、"打开微信"、"关闭这个窗口"、"最小化"、"最大化"
  - **表单填写**："帮我填一下这个表单"、"输入用户名密码"、"注册账号"、"登录进去"
  - **应用导航**："找到设置页面"、"在 Word 里插入图片"、"打开系统偏好设置"、"切换到 XX 应用"
  - **批量操作**："连续点击这些按钮"、"批量处理这些文件"、"把所有窗口关掉"
  - **视觉确认**："看看屏幕上有什么"、"帮我检查这个弹窗"、"确认一下登录成功没"、"现在显示的是什么"
  - **截图需求**："帮我截个图"、"把这个画面保存下来"、"截当前窗口"
  - **模糊 GUI 请求**："这个怎么用"、"帮我弄一下这个"、"点那个蓝色的按钮"、"关掉它"

  **边缘触发场景**（容易被忽略但应该触发）：
  - 用户说"稍等我看下屏幕"后请求帮助理解当前界面
  - 用户发送包含"怎么" + 应用名的请求（如"怎么在企业微信里发消息"）
  - 用户请求"演示"某个操作过程
  - 用户说"这个弹窗一直出来"等描述界面状态的问题
  - 用户请求"帮我试试"某个按钮或功能

  【何时**不**触发此技能】
  以下情况**不应该**触发此技能，应使用其他工具或直接回答：
  - **纯代码任务**："写个函数"、"改配置"、"修复这个 bug"（无 GUI 操作）
  - **文件操作**："读取这个文件"、"修改配置文件"（用文件工具而非屏幕控制）
  - **终端命令**："运行 npm install"、"执行这个脚本"（用 Bash 工具）
  - **网页内容查询**："查一下今天天气"（用浏览器工具或搜索，而非屏幕截图）
  - **概念性问题**："Python 是什么"、"解释一下这个概念"
  - **Creative 任务**："写篇文章"、"生成图片"

  【核心能力】
  - 多策略精确定位：Accessibility API / OCR / 颜色特征 / 坐标估算兜底 / AI 视觉
  - 执行前校验闭环：移动鼠标 → AI 确认位置 → 对才点击/错就修正
  - 时序帧感知：等待页面加载、检测操作生效
  - 系统级应用启动：使用 `open -a` 命令可靠打开应用

  【前置条件 - 必须满足】
  本技能依赖 Claude 的 Read 工具能够查看截图。如果 Read 无法显示图片内容（返回空或报错），
  则本技能**不可用**，应立即告知用户并停止操作。

  macOS 权限要求：
  - 屏幕录制权限：系统偏好设置 → 安全性与隐私 → 隐私 → 屏幕录制
  - 辅助功能权限：系统偏好设置 → 安全性与隐私 → 隐私 → 辅助功能（使用 Accessibility API 时）
---
