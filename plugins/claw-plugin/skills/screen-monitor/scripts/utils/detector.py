...
class ChangeDetector:
    ...
    def detect_change(self, current_hash: int) -> bool:
        ...
        distance = self.hamming_distance(self.prev_hash, current_hash)
    ...
    def is_stable(self, hashes: List[int], window: int = 3) -> bool:
        ...
        for i in range(len(recent) - 1):
            dist = self.hamming_distance(recent[i], recent[i + 1])
...
class FrameBuffer(deque):
    ...
    def get_keyframe(self) -> Optional[FrameData]:
        ...
        for i in range(len(self) - 2, -1, -1):
            dist = self.detector.hamming_distance(hashes[i], hashes[i + 1])
            if dist > 3:
                return self[i + 1]

        return self[-1]

    def get_recent(self, count: int) -> List[FrameData]:
        """获取最近的帧序列"""
        return list(self)[-count:]

    def get_changed_frames(self) -> List[FrameData]:
        """获取所有变化帧"""
        return [f for f in self if f.changed]
